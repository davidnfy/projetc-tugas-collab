<div class="p-8 bg-gray-50 min-h-full pb-24 transition-opacity duration-200 ease-in-out">
    <h1 class="text-2xl font-bold mb-6">🗓️ Daily Tasks</h1>

    {{-- Form Tambah Task --}}
    <form id="daily-form" action="{{ route('daily.store') }}" method="POST" class="mb-6">
        @csrf
        <div class="flex items-center bg-[#f3fdfd] border border-gray-200 rounded-lg shadow-sm px-4 py-3">
        ...
        @forelse ($todos as $todo)
            <div class="bg-white p-4 rounded-lg shadow flex justify-between items-center hover:shadow-md transition">
                ...
                <div class="flex gap-3">
                    <form action="{{ route('daily.toggle', $todo->id) }}" method="POST" class="toggle-form inline">
                        @csrf
                        @method('PATCH')
                    ...
                    <form action="{{ route('daily.destroy', $todo->id) }}" method="POST" class="delete-form inline">
                        @csrf
                        @method('DELETE')
